...
translate([-5, 0, 0]) difference()
{
    ...
    // slot
    translate([-(6+center_adjustment),33,8+center_adjustment]) cube([2,25,10]);
...
//    translate([4,35]) cube([10,20,20]);
    ...
    //PINDA mounting hole
    translate([-(5.5+center_adjustment),60,6+center_adjustment])rotate([90,0,0])cylinder(r=probe_width/2+0.2, h=30, $fn=20);
    translate([-(5.5+center_adjustment),41,6+center_adjustment])rotate([90,0,0])cylinder(r=probe_width/2+0.4, h=5, $fn=20);
    ...
    for ( i = [0 : 14] )
        {
          translate([-(5.5+center_adjustment),39+i,6+center_adjustment])rotate([90,0,0])cylinder(r1=probe_width/2+0.5, r2=probe_width/2-1, h=1, $fn=20);
          translate([-(5.5+center_adjustment),40+i,6+center_adjustment])rotate([90,0,0])cylinder(r1=probe_width/2-1, r2=probe_width/2+0.5, h=1, $fn=20);
        }
