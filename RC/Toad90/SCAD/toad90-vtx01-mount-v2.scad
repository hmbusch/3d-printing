/**
 * Eachine VTX01 mount for Toad90 frame (version 2)
 *
 ...
 * space and weight.
 *
 * Version 2 differs from version 1 in that the RX antenna tube comes
 * out the back, not the top. This makes the tube more resilient 
 * against crashes where the quad flips over.
 *
 */
    ...
    difference() {
        union() {
            ...
            hull() {
                ...
                translate([23.6 - 6/2, 18.6 + 1 + 6/2, 0]) cylinder(d = 6, h = 4 + zLift);
            }
            hull() {
                translate([23.6 - 6/2, 16.6 + 6/2, 4 + zLift]) sphere(d = 6);                
                translate([23.6 - 6/2, 18.6 + 3 + 6/2, 4 + zLift]) sphere(d = 6);                
            }
        }
        // cutout for cables
        ...
        translate([23.6 - 6/2, 17.6 + 1 + 6/2 + 2, 2 + zLift]) 
            rotate([90, 0, -90]) 
            rotate_extrude(angle = 90, convexity = 10, $fn = 128) 
            translate([2, 0, 0]) 
            circle(d = 3.1);
        translate([23.6 - 6/2, 17.6 + 1 + 6/2, -0.01]) cylinder(d = 3.1, h = 2.2 + zLift);
        translate([23.6 - 6/2, 17.6 + 6/2 + 2.99, 4 + zLift]) rotate([-90, 0, 0]) cylinder(d = 3.1, h = 5);
